...
// Ajustar conforme o balde usado
...
// Obs: 18cm x 20cm dá ~5.1 L

static const float SENSOR_OFFSET_CM = 0.0f;

// ===== CONFIG MEDIÇÃO =====
static const uint32_t TIMEOUT_US = 25000;     // ~25ms
static const float SPEED_CM_PER_US = 0.0343f; // velocidade do som ~343 m/s => 0.0343 cm/us

// SENSOR ROXO CO2
static const int I2C_SDA = 21; // SDA
static const int I2C_SCL = 22; // SCL
...
// ======================================================
// FUNÇÃO: LER ADC FAZENDO MÉDIA
// ======================================================
// Faz várias leituras analógicas e retorna a média para
// reduzir ruído dos sensores MQ.

uint16_t readADC_Avg(int pin, int samples = 20, int delayMs = 5)
{
  uint32_t sum = 0; // variável para acumular as leituras
  for (int i = 0; i < samples; i++)
  {
    sum += analogRead(pin); // lê o valor analógico do pino
    delay(delayMs);         // pequena pausa entre leituras
  }
...
float measureDistanceCm()
{
  ...
  // Mede o tempo que o pino ECHO ficou HIGH
  uint32_t duration = pulseIn(ECHO_PIN, HIGH, TIMEOUT_US);
  // Se deu timeout (nenhum eco), retorna erro
  if (duration == 0)
    return -1.0f;
...
// ======================================================
// FUNÇÃO: EXTRAIR VALOR FLOAT DE UM PACOTE STRING
// ======================================================
// Exemplo de pacote:
// "temp:23.5;ph:7.1;tds:200;"
...
// ======================================================
// FUNÇÃO: EXTRAIR VALOR INT DE UM PACOTE STRING
// ======================================================
...
void setup()
{
  ...
  // ==================================================
  // CONFIGURAÇÃO DO ADC (SENSORES MQ)
  // ==================================================
...
void loop()
{
  ...
  if (distance_cm >= 0)
  {
    ...
    if (liquid_height_cm < 0)
      liquid_height_cm = 0;
    if (liquid_height_cm > BUCKET_HEIGHT_CM)
      liquid_height_cm = BUCKET_HEIGHT_CM;
  ...
  if (ccs.available())
  {
    if (!ccs.readData())
    {
      ...
      // nomaliza valor para porcentagem
      float co2_percent =
  ...
  if (millis() - ultimoEnvio >= intervaloEnvio)
  {
    ...
    // porcentagem
    float co2 = last_co2_percent; // aqui é o CO2%
    float nivel_litros = volume_L;

    // Envia dados para ESP2
    String pacoteESP1 =
