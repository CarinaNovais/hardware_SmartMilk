...
// ================= pH =================
float V_PH7  = 2.50;   // AJUSTE quando usar solução pH 7
float PH_GAIN = 3.5;
...
float calcularTDS(float v) {
  if (v < V_MIN_VALIDO) return 0;
  float coef = 1.0 + 0.02 * (temperaturaC - 25.0);
  float v_comp = v / coef;
  float tds = 133.42 * pow(v_comp, 3)
            - 255.86 * pow(v_comp, 2)
            + 857.39 * v_comp;
  tds *= FATOR_CAL_TDS;
  if (tds < 0) tds = 0;
...
float calcularTurbidez(float v) {
  if (v >= V_CLEAR) return 0;
  if (v <= V_TURVA) return NTU_TURVA;
