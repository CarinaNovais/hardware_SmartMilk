...
void loop() {
  ...
  // Proteção contra NaN
  float perc_CH4 = 0;
  if (R0_MQ2 > 0 && Rs_MQ2 > 0) perc_CH4 = getGasPercentage(Rs_MQ2, R0_MQ2);

  float perc_NH3 = 0;
  if (R0_MQ135 > 0 && Rs_MQ135 > 0) perc_NH3 = getGasPercentage(Rs_MQ135, R0_MQ135);
...
float getGasPercentage(float Rs, float R0) {
  float ratio = Rs / R0;
  float A = 100.0;
  float B = -1.2; // ajuste se precisar maior precisão
